...
import com.example.basketballscoreboard.Player;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void showDatabase(String sort){
        ...
        while(iCursor.moveToNext()){
            ...
            String tempAttempts = iCursor.getString(iCursor.getColumnIndex("attempts"));
            tempAttempts = setTextLength(tempAttempts,10);

            String tempSuccess = iCursor.getString(iCursor.getColumnIndex("success"));
            tempSuccess = setTextLength(tempSuccess,10);

            //String Result = tempID + tempName + tempAge + tempGender;
            String Result = tempID + tempName + tempAge+ tempAttempts+ tempSuccess;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_insert:
                ...
                attempts = Long.parseLong(edit_Age.getText().toString());
                success = Long.parseLong(edit_Age.getText().toString());
                mDbOpenHelper.open();
                mDbOpenHelper.insertColumn(ID, name, age,attempts,success);
                showDatabase(sort);
                setInsertMode();
            ...
            case R.id.btn_update:
                ...
                mDbOpenHelper.updateColumn(nowIndex,ID, name,age);
                showDatabase(sort);
                setInsertMode();
                edit_ID.requestFocus();
                edit_ID.setCursorVisible(true);
